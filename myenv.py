...
class MyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.kaclang <= 0:
          if action == 0:
            dmg = np.floor((MyEnv.erdrickStrength - MyEnv.zomaDefence / 2) * dq3random(np.random))
            if debugLog:
              print ("あるすの こうげき！")
            ...
            if self.erdrickMana >= manaCost:
              if debugLog:
                print ("あるすは ホイミを となえた！")
              self.erdrickHealth += np.floor(np.random.random() * 10) + 30
              self.erdrickMana -= manaCost
            else:
              manaLack = True
          elif action == 6:
            ...
            if self.erdrickMana >= manaCost:
              if debugLog:
                print ("あるすは ベホイミを となえた！")
              self.erdrickHealth += np.floor(np.random.random() * 20) + 75
              self.erdrickMana -= manaCost
            else:
              manaLack = True
          elif action == 7:
        ...
        if self.zomaHealth > 0 and self.kaclang <= 0:
          for i in range(2):
            ...
            dmg = 0
            if zomaAction <= 0.375:
              dmg = np.floor((MyEnv.zomaStrength - MyEnv.erdrickDefence / 2) * dq3random(np.random))
              if defendFlag:
                dmg = np.floor(dmg / 2)
              if debugLog:
                print ("ゾーマの こうげき！")
